...
class degree {
  ...
  public function insert($id_degree_level, $code, $fullName, $destination) {
    if ($this->valid($code, $fullName)) {
      try {
        ...
        $db = new database();
        $con = $db->getCon();
        $sql = 'INSERT INTO tDegrees (id_degree_level, code, fullName) VALUES ( :i, :c, :n)';
  ...
  public function insertLevel($designation, $destination) {
    if ($this->validLevel($destination)) {
      try {
        ...
        $db = new database();
        $con = $db->getCon();
        $sql = 'INSERT INTO tDegreeLevels (designation) VALUES ( :d )';
        $data = $con->prepare($sql);
  ...
  public function update($id_degree, $id_degree_level, $code, $fullName, $destination) {
    if ($this->valid($code, $fullName)) {
      try {
        ...
        $db = new database();
        $con = $db->getCon();
        $sql = 'UPDATE tDegrees SET id_degree_level = :i, code = :c, fullName = :f WHERE id_degree = :id';
        $data = $con->prepare($sql);
        $data->bindvalue(':i', $id_degree_level);
        $data->bindvalue(':c', $code);
  ...
  public function updateLevel($id_degree_level, $designation, $destination) {
    if ($this->validLevel($designation)) {
      try {
        ...
        $db = new database();
        $con = $db->getCon();
        $sql = 'UPDATE tDegreeLevels SET designation = :d WHERE id_degree_level = :id';
        $data = $con->prepare($sql);
        $data->bindvalue(':d', $designation);
  ...
  public function delete($id, $destination) {
    try {
      ...
      $db = new database();
      $con = $db->getCon();
      $sql = 'DELETE FROM tDegrees WHERE id_degree = :i';
      $data = $con->prepare($sql);
      $data->bindvalue(':i', $id);
      $data->execute();
      if ($destination != null) header('Location:' . $destination);
  ...
  public function deleteLevel($id, $destination) {
    try {
      ...
      $db = new database();
      $con = $db->getCon();
      $sql = 'DELETE FROM tDegreeLevels WHERE id_degree_level = :i';
      $data = $con->prepare($sql);
      $data->bindvalue(':i', $id);
      $data->execute();
      if ($destination != null) header('Location:' . $destination);
  ...
  public function fetch($id) {
    try {
      ...
      $db = new database();
      $con = $db->getCon();
      $sql = 'SELECT id_degree_level, code, fullName FROM tDegrees WHERE id_degree = :i';
      $data = $con->prepare($sql);
      $data->bindvalue(':i', $id);
      $data->execute();
      $degree = $data->fetch();
  ...
  public function degrees($id_degree_level = -1) {
    try {
      ...
      $db = new database();
      $con = $db->getCon();
      $sql = 'SELECT tDegrees.id_degree, tDegreeLevels.designation, tDegrees.code, tDegrees.fullName FROM tDegrees, tDegreeLevels WHERE tDegrees.id_degree_level = tDegreeLevels.id_degree_level AND (tDegrees.id_degree_level = :idl OR :idl = -1) ORDER BY tDegrees.id_degree_level, tDegrees.fullName';
      $data = $con->prepare($sql);
  ...
  public function levelDepend($id_degree_level) {
    try {
      ...
      $db = new database();
      $con = $db->getCon();
      $sql = 'SELECT id_degree FROM tDegrees WHERE id_degree_level = :id';
      $data = $con->prepare($sql);
      $data->bindvalue(':id', $id_degree_level);
      $data->execute();
      $degree = $data->fetch();
  ...
  //level enum
    ...
    try {
    	...
    	$db = new database();
    	$con = $db->getCon();
    	$sql = 'SELECT id_degree_level, designation FROM tDegreeLevels ORDER BY id_degree_level';
    	$data = $con->prepare($sql);
    	$data->execute();
